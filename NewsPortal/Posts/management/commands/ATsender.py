...
def my_job():
    # Получение последних 3 статей за последнюю неделю.
    only_article = Post.objects.filter(article_or_news__istartswith="AT").dates("datetime", "week").values()[:3]

    # Индекс категории, о которой вы хотите отправлять электронные письма.
    choose_category = 0
    # Он получает идентификатор первой категории.
    category_number = Category.objects.all().values()[choose_category]["id"]
    # Он получает название категории.
    category_name = Category.objects.all().values()[category_number - 1]["name_category"]

    # Запрос Django ORM, который получает все электронные письма пользователей, у которых есть подписка с категорией.
    get_emails = list(User.objects.filter(subscriptions__category=category_number).values_list('email', flat=True))

    # Здесь я мог бы использовать EmailMultiAlternatives, выучу html и приведу порядок. :(
